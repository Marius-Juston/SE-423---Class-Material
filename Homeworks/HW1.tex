...
\item Build and run this code on your Launchpad (red board). Does everything work correctly? Check TeraTerm for the prints. \textbf{Demo this code to your TA in person / over video.} The print out should look something like
\item 
\begin{minted}{c}
...
Etc...
\end{minted} 
\item A common mistake with \texttt{serial\_printf} is to use the \%d formatter for 32 bit integers. (\%d is for 16-bit integers only on the F28379D processor.)  In the \texttt{serial\_printf} statement, change the \%ld formatter to \%d and see what happens. \textbf{Also, demo this to your TA in person/ over video}. 
