...
\def\AssignmentText{The Demonstration Check-Offs For Exercises 2, 3, 5, 7, 8, 9 And 10 Are Due By \textcolor{highlight}{\HWCheckOffTime~\GetDate{2}{1}}. \par
...
%
\begin{enumerate}
    \item Solder the four 2X10 gold-plated headers. \textbf{Show your instructor that you have them in the correct orientation before soldering} 
...
\centering
\Annotated[width=\linewidth]{Homeworks/Figures/HW1/Board2.jpg}{%
    Soldered breakout board, top-left view. Two red ellipses highlight
    the locations where a long wire (upper) and a short wire (lower)
    bridge between header pins on the left edge of the board.%
}{%
...
\centering
\Annotated[width=\linewidth]{Homeworks/Figures/HW1/Board1.jpg}{%
    Top-down view of the centre region of the soldered breakout
    board. Five red circles mark the soldering points: four around
    the chip and surface-mount components in the middle, and one on
    the left near the buzzer header.%
}{%
...
\begin{subfigure}[b]{0.245\textwidth}
\centering
\Annotated[width=\linewidth]{Homeworks/Figures/HW1/Board3.jpg}{%
    Top-right view of the soldered breakout board. A single red
    ellipse marks the placement of a short jumper wire on the right
    edge of the board.%
}{%
...
Write a global function and name it “\texttt{saturate}”. Do not use any global variables in this function. This function should have two \mintinline{c}{float} parameters, “\texttt{input}” and “\texttt{saturation\_limit}”, and return a \mintinline{c}{float}. This return value is saturated by “\texttt{saturation\_limit}”. “\texttt{input}” is the input signal (or input value) that will be saturated if its value is greater than +\texttt{saturation\_limit} or less than –\texttt{saturation\_limit}. For example, if “\texttt{input}” is passed 3.4 and “\texttt{saturation\_limit}” is passed 5.5, then “\texttt{saturate}” will return 3.4. If “\texttt{input}” is passed 9.5 and “\texttt{saturation\_limit}” is passed 5.5, then “\texttt{saturate}” will return 5.5. If “\texttt{input}” is passed -7.4 and “\texttt{saturation\_limit}” is passed 5.5, then “\texttt{saturate}” will return -5.5. You will test this function in Exercise 8.  
...
\item As a final step, modify the \texttt{serial\_printf} statement so that it prints on one line: timeint, time with 2 digits of precision, \verb|sinvalue| with 3 digits of precision, and \verb|satvalue| with 2 digits of precision.  \%.3f is the formatter for printing a float with 3 digits of precision. \%ld is the formatter to print a 32-bit integer.   
\item Build and run this code on your Launchpad (red board). Does everything work correctly? Check TeraTerm for the prints. \textbf{Demo this code to your TA in person.} The print out should look something like
\item 
\begin{minted}{c}
...
Etc...
\end{minted} 
\item A common mistake with \texttt{serial\_printf} is to use the \%d formatter for 32 bit integers. (\%d is for 16-bit integers only on the F28379D processor.)  In the \texttt{serial\_printf} statement, change the \%ld formatter to \%d and see what happens. \textbf{Also, demo this to your TA in person}. 
