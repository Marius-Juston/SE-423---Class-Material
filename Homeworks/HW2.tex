...
\def\AssignmentText{The Demonstration Check-Offs For Exercises 1 And 2 Are Due By \textcolor{highlight}{\HWCheckOffTime~\GetDate{4}{1}}. Check-Offs For Exercises 3 And 4 Are Due By \textcolor{highlight}{\HWCheckOffTime~\GetDate{5}{1}}. \par
Remainder Due On GradeScope, by \textcolor{highlight}{\HWSubmitTime~\GetDate{5}{2}}.}

%%%%%%%%%%%%%%%%%%%%%%%%%%%

\definecolor{bitbg}{RGB}{255,255,210}
...
\centering
\AltStart{%
    Voltage-divider schematic. A light-dependent resistor (LDR) sits
    between VCC at the top and a junction node in the middle; a
    fixed 2.2 kilohm resistor sits between that junction and GND at
    the bottom. The junction connects to an ADC pin on the left.%
}
\begin{tikzpicture}
...
\centering
\begin{tblr}{
  colspec = {*{8}{X[c,m]}}, % 8 centered columns, evenly spaced
  hlines, vlines,           % Automatically draw all grid lines
  row{1,4} = {bg=bitbg}     % Apply background color to rows 1 and 4
}
15 & 14 & 13 & 12 & 11 & 10 & 9 & 8 \\
\SetCell[c=2]{c} FREE\_SOFT & & PHSDIR & \SetCell[c=3]{c} CLKDIV & & & \SetCell[c=2]{c} HSPCLKDIV & \\
\SetCell[c=2]{c} R/W-0h & & R/W-0h & \SetCell[c=3]{c} R/W-0h & & & \SetCell[c=2]{c} R/W-1h & \\
7 & 6 & 5 & 4 & 3 & 2 & 1 & 0 \\
HSPCLKDIV & SWFSYNC & \SetCell[c=2]{c} SYNCOSEL & & PRDLD & PHSEN & \SetCell[c=2]{c} CTRMODE & \\
R/W-1h & R-0/W1S-0h & \SetCell[c=2]{c} R/W-0h & & R/W-0h & R/W-0h & \SetCell[c=2]{c} R/W-3h & \\
\end{tblr}
\end{table}
%
and
...
\begin{tblr}{
  ...
  hlines, vlines,
  row{1,4} = {bg=bitbg},
  cell{2}{1} = {bg=gray!30} % Target the RESERVED cell (Row 2, Column 1)
}
15 & 14 & 13 & 12 & 11 & 10 & 9 & 8 \\
\SetCell[c=4]{c} RESERVED & & & & \SetCell[c=2]{c} CBD & & \SetCell[c=2]{c} CBU & \\
\SetCell[c=4]{c} R-0-0h & & & & \SetCell[c=2]{c} R/W-0h & & \SetCell[c=2]{c} R/W-0h & \\
7 & 6 & 5 & 4 & 3 & 2 & 1 & 0 \\
\SetCell[c=2]{c} CAD & & \SetCell[c=2]{c} CAU & & \SetCell[c=2]{c} PRD & & \SetCell[c=2]{c} ZRO & \\
\SetCell[c=2]{c} R/W-0h & & \SetCell[c=2]{c} R/W-0h & & \SetCell[c=2]{c} R/W-0h & & \SetCell[c=2]{c} R/W-0h & \\
\end{tblr}
\end{table}
%
...
\textbf{Show the TAs on the oscilloscope that the LED1 is going up and down every 200 us.}
%
\begin{figure}[H]
    \centering
\AltStart{%
    Triangular brightness waveform: PWM duty cycle plotted against
    time. The signal ramps linearly from 0 percent up to 100 percent
    over the first half of the period, then back down to 0 percent
    over the second half, repeating to produce a continuous breathe
    effect on the LED.%
}
...
//set to last or only SOC that is converted, and it will set INT1 flag ADCA1
...
\textbf{Demonstrate these items working for your check off.} 
...
Using the readings of the Joystick X and Y, write code to update LEDs to point in the direction you have moved the Joystick. If Joystick Y is in the center position, turn on only LED8. If Joystick Y is moved past a threshold and pointing towards the buzzer, then turn on only LED6. If Joystick Y is moved past another threshold and pointing away from the buzzer, then turn on only LED10. Similarly, if Joystick X is in the center position, turn on only LED8 if Joystick X is moved past a threshold, and, pointing down, turn on only LED13. Suppose Joystick X is moved past a threshold and pointing up, then only LED3 turns on.
...
\begin{figure}
    \centering
    % tex4ht's PDF-to-PNG fallback misnames the output for single-page
    % vector PDFs; ship a sibling SVG (regenerated in CI from SPI.pdf)
    % and use it in HTML mode so the timing diagram renders without
    % losing vector fidelity.
    \AltStart{%
        Four-channel SPI timing diagram showing MOSI, MISO, SCK and
        Slave Select waveforms over roughly 35 microseconds. SS is
        active-low and pulses low for each of the six byte transfers;
        SCK toggles eight times within each SS-low window. MOSI and
        MISO each carry six bytes of data, transmitted MSB-first.%
    }%
    \ifSEhtml
      \includegraphics[width=1\linewidth]{Homeworks/Figures/HW2/SPI.svg}%
    \else
      \includegraphics[width=1\linewidth]{Homeworks/Figures/HW2/SPI.pdf}%
    \fi
    \AltEnd
