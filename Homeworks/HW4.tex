...
\def\AssignmentText{The Demonstration Check-Offs For Exercises 2 and 3 Are Due By \textcolor{highlight}{\HWCheckOffTime~\GetDate{10}{1}}. \par
The Demo Check-Offs for Exercise 4 are due by \textcolor{highlight}{\HWCheckOffTime~\GetDate{11}{1}}. \par
Remainder Due On Gradescope, by \textcolor{highlight}{\HWSubmitTime~\GetDate{11}{2}}.}

% If you want to change the caption names
...
\centering
\Annotated[width=\linewidth]{Homeworks/Figures/HW4/Solder1.jpg}{%
    Close-up of the breakout board's right edge, with a thick red
    ellipse highlighting the pair of through-holes where the AA
    battery pack's positive and negative leads must be soldered.%
}{%
...
\centering
\Annotated[width=\linewidth]{Homeworks/Figures/HW4/Solder2.jpg}{%
    Wider view of the demo breakout board after the battery pack
    leads have been soldered, with a thick red ellipse around the
    central solder joints to indicate the finished battery
    connection.%
}{%
...
\centering
\AltStart{%
    Two coordinate frames in the plane. Inertial frame I has its
    origin in the lower-left, x pointing right and y pointing up.
    Body frame J is a rectangle placed above and to the right of I,
    with its x-axis pointing down and its y-axis pointing right —
    so frame J is rotated minus ninety degrees relative to I.%
}
...
\centering
\AltStart{%
    Inertial frame I in the upper-right with its standard x-right,
    y-up axes. Body frame J is a rectangle in the lower-left whose
    x-axis points left and y-axis points down — frame J is rotated
    one hundred eighty degrees relative to I.%
}
\begin{tikzpicture}[>=Stealth, scale=1.2]
    % Inertial Frame I
    \draw[->, thick] (0,0) node[below left] {$I_o$} -- (2,0) node[right] {$I_x$};
...
\centering
\AltStart{%
    Body frame J at an arbitrary angle theta from inertial frame I.
    A dashed reference line and a dashed arc labelled theta indicate
    the rotation angle measured from frame I's x-axis to frame J's
    x-axis, with the body shown rotated counter-clockwise by roughly
    two hundred twenty-five degrees in the example diagram.%
}
\begin{tikzpicture}[>=Stealth, scale=1.2]
    % Inertial Frame I
    \draw[->, thick] (0,0) node[below left] {$I_o$} -- (2,0) node[right] {$I_x$};
...
%
\begin{enumerate}
    \item $\overrightarrow{I_0J_0} = (3.83, -1.75, 0)$ (associate with \ref{fig:first_rotation})
    \item $\overrightarrow{I_0J_0} = (11.1,6.7,0)$ (associate with \ref{fig:second_rotation})
    \item $\overrightarrow{I_0J_0} = (5.3,-1.4,0)$ (associate with \ref{fig:third_rotation})
\end{enumerate}
%
Moreover, using the results on the previous \ref{fig:first_rotation}, \ref{fig:second_rotation}, and \ref{fig:third_rotation} questions respectively, \textbf{obtain the homogeneous transformation matrix}, which converts frame $J$ coordinates to frame $I$ coordinates.
...
\begin{figure}[H]
    \centering
\AltStart{%
    Schematic of a TIP122 NPN transistor wired as a low-side switch.
    The GPIO drives the transistor base through a base resistor RB.
    The collector connects to an ultra-bright LED in series with a
    47-ohm current-limit resistor, both powered from the +6V rail.
    The emitter ties to ground (routed for the student).%
}
\begin{tikzpicture}
...
\begin{figure}[H]
    \centering
\AltStart{%
    Schematic of the TIP122 transistor driving a DIP05-1A57 reed
    relay in parallel with a fly-back diode (1N4003). The relay's
    coil is energised when the GPIO turns the transistor on; the
    relay's normally-open contacts then close, completing a +6V
    circuit through a standard LED in series with a 47-ohm
    resistor. Pin numbers 1 through 14 of the DIP package are
    labelled around the relay symbol.%
}
\begin{tikzpicture}
	% Paths, nodes, and wires:
...
\begin{figure}[H]
    \centering
    \AltStart{%
        Top-down diagram of the robot's LIDAR field of view: a fan
        of sixteen radial wedges spanning two hundred forty degrees
        in front of a small central LiDAR unit, from minus one
        hundred twenty degrees on the robot's right to plus one
        hundred twenty degrees on its left. Dashed lines mark the
        forward and side axes; an arrow labelled "LiDAR (small
        circle)" points to the sensor at the centre.%
    }
    \begin{tikzpicture}
...
This homework assignment is up to you. Use your creativity to build a simple mechatronics project that uses \textbf{at least one RC servo motor} and one sensor of your choice. You can use the actuators and sensors we have used in homework up to this point. Choose any one of the following:
\begin{itemize}
    ...
    \item Microphone
\end{itemize}
Anything goes, but keep in mind that you also have a final project with the robot to complete by the end of the semester. So, in other words, we are not expecting an elaborate, finely polished design. \textbf{You cannot use the LADAR or the lab robot for the project. It needs to be using the homework board.}
