...
% Course metadata
\input{header}
\def\AssignmentText{Due On Gradescope, by \textcolor{highlight}{\HWSubmitTime~\GetDate{15}{4}}.}
...
\begin{enumerate}
    ...
    \item As a final exercise, add enough obstacles so that there is no path A* can find from the starting point to the ending point. Verify that A* returns an error when it cannot find a path. Make sure to show the map used, not just the error.
\end{enumerate}
%
\Ex[Kalman filtering]
...
\begin{itemize}
    ...
    \item Three plots showing the zoomed in discontinuity in the OptiTrack data associated with the previous $Q$ and $R$ pairs.
\end{itemize}
%
