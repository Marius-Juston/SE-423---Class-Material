...
% Course metadata
\input{header}
\def\AssignmentText{This is an optional assignment if you would like to learn about how LABVIEW can be used like MATLAB/Simulink to simulate system equations.}
%
\begin{document}
...
\begin{figure}[H]
    \centering
    \AltImg[width=1\linewidth]{Homeworks/Figures/LabVIEW4/Picture1.png}{%
        Reference VI front panel: a SimTime Waveform graph plots two
        signals (the system response and the friction-compensated
        response) over time, with numeric controls for gain, friction
        deadband, and a stop button.%
    }
\end{figure}
\begin{figure}[H]
    \centering
    \AltImg[width=1\linewidth]{Homeworks/Figures/LabVIEW4/Picture2.png}{%
        Block diagram of the reference VI inside a Control and
        Simulation Loop: a transfer-function block, a derivative
        approximation 100s over (s+100), the Friction Compensation
        sub-network, and a Build Array feeding the SimTime Waveform
        graph.%
    }
\end{figure}
...
\begin{figure}[H]
    \centering
    \AltImg[width=0.5\linewidth]{Homeworks/Figures/LabVIEW4/Picture3.png}{%
        SubVI icon and connector pane layout: a small custom icon,
        with the connector pane wired so the simulation input is on
        the left terminal and the friction-compensated output is on
        the right terminal.%
    }
...
\begin{figure}[H]
    \centering
    \AltImg[width=1\linewidth]{Homeworks/Figures/LabVIEW4/Picture4.png}{%
        Block diagram with the friction-compensation logic factored
        out into a single SubVI tile, leaving the rest of the
...
\begin{figure}[H]
    \centering
    \AltImg[width=1\linewidth]{Homeworks/Figures/LabVIEW4/Picture5.png}{%
        Block diagram variant where the friction-compensation SubVI
        has been replaced by a Formula Node containing equivalent C
        code; inputs and an output are added on the node's left and
