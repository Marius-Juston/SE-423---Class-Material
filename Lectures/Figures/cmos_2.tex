...
\begin{tikzpicture}[
    ...
    % Y-Coordinates mapping relative to the grid
    \pgfmathsetmacro{\colAmpsY}{-\gap} 
    \def\colAmpH{1.7}      % Height of the new column amp circuitry section
    \def\colAmpB{-0.2}
    \pgfmathsetmacro{\colMuxY}{\colAmpsY - \colAmpH - \gap} 
    \pgfmathsetmacro{\sensorBottom}{\colMuxY - \blkH}
    ...
    \foreach \x in {1,...,\Ncols} {
        \pgfmathsetmacro{\px}{(\x-1)*\dx}
        % Column Bus extending out of the array into the amp section
        \draw[busCol] (\px+1.2, \colAmpsY) -- (\px+1.2, \Harray-0.1);
    }
    ...
    % ==========================================
    % 3. COLUMN BIAS & SAMPLING (Layer D)
    % ==========================================
    \draw[dashedbox, fill=mLightBlue!30] (0, \colAmpsY) rectangle (\Warray, \colAmpsY-\colAmpH);
    

    % Global Vbias line
    \draw[mDarkBlue] (0, \colAmpsY-0.9+ \colAmpB) node[left] {$v_{bias}$} -- (\Warray-0.1, \colAmpsY-0.9+ \colAmpB);

    \foreach \x in {1,...,\Ncols} {
        \pgfmathsetmacro{\px}{(\x-1)*\dx}

                % Routing to Column Mux (Bypassing the bias transistor)
        \draw[busCol] (\px+1.2, \colAmpsY-0.3+ \colAmpB) -- (\px+1.35, \colAmpsY-0.3+ \colAmpB) -- (\px+1.35, \colMuxY+ \colAmpB);
        
        % Bias Transistor (Active Load)
        \node[nmos] (Bias_\x) at (\px+1.2, \colAmpsY-0.9+ \colAmpB) {};
        \draw[busCol] (\px+1.2, \colAmpsY+ \colAmpB+0.2) -- (Bias_\x.D); 
        \draw (Bias_\x.S) -- (\px+1.2, \colAmpsY-1.1+ \colAmpB) node[ground, line width=0.1]{};
        \draw (Bias_\x.G) node[connector]{}; % Tap into Vbias
        
        % Column Bus Capacitor (C_b)
        \draw (\px+1.2, \colAmpsY-0.3+ \colAmpB) node[connector]{} -- (\px+0.45, \colAmpsY-0.3+ \colAmpB) 
              to[C] (\px+0.45, \colAmpsY-0.6+ \colAmpB) node[ground, line width=0.1]{};
              
    }
    \node[anchor=north west, font=\bfseries] at (0, \colAmpsY) {Column Bias \& Sampling};
