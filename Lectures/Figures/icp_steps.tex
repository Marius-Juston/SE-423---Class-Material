...
\node[font=\small] at (axis cs: 1.5, -1.5) {H = \(\sum_i (p_i-\bar{p})(q_i-\bar{q})^T\)};
\node[font=\small] at (axis cs: 1.5, -1.8) {(Conceptual centering shown)};
...
\node[cyan, font=\large] at (axis cs: 3, 3) {\(P_{aligned} = R(P-\bar{p}) + \bar{q}\)};
